...
type WordleResult struct {
	...
	Seconds    float64
	GuessCount int
	// One result per account per day, played by one of its characters
	PlayerID    int
	CharacterID int
}
...
// Whether any of the player's characters played on date
...
func (s *Store) InsertWordle(r WordleResult) error {
	...
	_, err := s.db.Exec(`
	INSERT INTO Wordle (date, win, seconds, guessCount, player_id, character_id)
	VALUES (?, ?, ?, ?, ?, ?);
	`, r.Date, win, r.Seconds, r.GuessCount, r.PlayerID, r.CharacterID)
	return err
}
...
func (s *Store) WordleLeaderboard(date string, limit int, offset int) ([]LeaderboardRow, int, error) {
	...
	rows, err := s.db.Query(`
	SELECT p.username, c.character_id, c.name, c.class, w.guessCount, w.seconds FROM Wordle w
	INNER JOIN Character c ON w.character_id = c.character_id
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.Uname, &row.CharacterID, &row.Char, &row.Class, &row.Guesses, &row.Time); err != nil {
			return nil, 0, err
		}
		row.Place = place
