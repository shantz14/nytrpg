...
- **Ranked:** the elo math is pure (`internal/ranked`), so test it there. In world tests, `tw.rate(p, elo)` makes an established player and `tw.OnRanked` catches saved matches. `tw.profile(c, target)` flushes the world while `Profile` waits on it. Ratings are saved by `ranked.Service`, a single ordered worker: a `Load` after a `Record` always sees it, so reconnect tests don't need sleeps.
- **Schema rollback tests** (like `TestMigrationGivesOldPlayersACharacter`) drop every table from later migrations too. Add yours there when you append a migration.
